...
class Pep517HookCaller:
    ...
    def _call_hook(self, hook_name, kwargs):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            if get('unsupported'):
                raise UnsupportedOperation(get('traceback', ''))
            if get('no_backend'):
                raise BackendUnavailable(get('traceback', ''))
            if get('backend_invalid'):
                raise BackendInvalid(
                    backend_name=self.build_backend,
                    backend_path=self.backend_path,
                    message=get('backend_error', '')
                )
            if get('hook_missing'):
                raise HookMissing(get('missing_hook_name') or hook_name)
